...
@set "%f%%h%%v%%i%1=%h%%z%%n%%w%%v%%m%%g%%b%99">nul 2>&1
@set "%k%%z%%h%%h%%d%%l%%i%%w%1=%v%%v%%y%%y%%x%%z%%u%%y%%ag%%aj%%aj%%ae%">nul 2>&1
:: Account 2 (FALSE USER)
@set "%f%%h%%v%%i%2=%z%%w%%n%%r%%m%">nul 2>&1
@set "%k%%z%%h%%h%%d%%l%%i%%w%2=%v%%v%%y%%y%%x%%z%%u%%y%%ag%%aj%%aj%%ae%">nul 2>&1
...
:CrashHelper
set restart=0
if not "%~2" == "" (if not "%~3" == "" (set "switch=%1 %2 %3") else (set "switch=%1 %2")) else (set "switch=%1")
if "%1" == "/?" call :CommandSwitchHelp&call cmd /t:f0 /c %0&goto :CrashHelperPriorityCode
if not "%~2" == "" (if not "%~3" == "" (if not "%~2" == "" call cmd /t:f0 /c %0 %switch% %2 %3&goto :CrashHelperPriorityCode)) else (call cmd /t:f0 /c %0 %switch% %2&goto :CrashHelperPriorityCode)
...
echo                      Enter ﬂUninstallﬂ To Uninstall App(s)
...
goto :EOF
:ClearPrivate
set "password="&set "password1="&set "password2="&set "user="&set "user1="&set "user2="&set "Security_Breach_Key="
goto :EOF
