...
set SDS_Version=1.3.7B
set Version_Type=STABLE
set currentyear=2015&set "sds_file=%~nx0"&set "sds_extension=%~x0"&set "sds_folder=%~dp0"
...
goto CrashHelper
:: No Customization CODE ABOVE!! (This Is Priority Code / Safemode Booting Code)
:CrashHelperActive
...
if exist "%temp%\Enable_Hotspot.SD_PROGRESS_FILE" if "%adminPer%" == "yes" del %temp%\Enable_Hotspot.SD_PROGRESS_FILE&call :SetWindow&goto adminenable
if exist "%temp%\Enable_Hotspot2.SD_PROGRESS_FILE" if "%adminPer%" == "yes" del %temp%\Enable_Hotspot2.SD_PROGRESS_FILE&call :SetWindow&goto confirmedHot
call :plugins inboot
call :MakeFiles
...
if /i %Bug_101_Vuln%==yes call :SecTimer Stop>nul 2>&1&call :Bug101Yes
:EndBug101
...
echo    * Create ^& Publish Apps ^& Plugins For SD-Security
echo   -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
...
echo Û2Û  Yes (On This Folder, BETA Feature)
echo Û3Û  No
%set /p% showSDIcon="Ûþ "
...
if /i exist "SDS_Files\HideBootTime.ini" (%switch% switch6 on) else (%switch% switch6 off)
if /i exist "DisableCrashHelper.ini" (%switch% switch7 on) else (%switch% switch7 off)
...
echo Û8Û  Change Master Override Key
echo Û9Û  Change Application Colour Theme
echo.
echo ÛAÛ  Save Settings
echo ÛBÛ  Cancel (Discard Settings)
CHOICE /C 123456789AB /N /M "Ûþ "
IF ERRORLEVEL 11 %settheme%&goto :AdminOps
IF ERRORLEVEL 10 goto :SaveNewSdsSettings
IF ERRORLEVEL 9 call :ChangeTheme&goto :ChangeSettingsRefresh
IF ERRORLEVEL 8 call :ChangeBreachKey&goto :ChangeSettingsRefresh
...
for /f %%a in ('dir /O:-D /B /A:D SDS_Files\Plugins') do (echo Û %%a)
if "%customApp%" == "none" echo No Apps Installed, Press Any Key To Exit&pause >nul&goto :UninstallApp
%set /p% UninstallApp="Ûþ "
...
for /f %%a in ('dir /O:-D /B /A:D SDS_Files\Plugins') do (echo Û %%a)
if "%customApp%" == "none" echo No Apps Installed, Press Any Key To Exit&pause >nul&goto AppInfo
%set /p% AppInfo="Ûþ "
...
title Custom App '%~1'
call :ClearPrivate
if "%~1" == "" (goto :EOF) else (if exist "SDS_Files\Plugins\%~1\home.bat" (set "FullAppFolder=%cd%\SDS_Files\Plugins\%~1"&set "AppFolder=SDS_Files\Plugins\%~1"&call SDS_Files\Plugins\%~1\home.bat))
endlocal
goto :EOF
:ClearPrivate
...
:AdminCMDRunCheck
call :AdminCMDCheck
if /i not "%AdminCMDStatus%"=="Active" call :AdminCMDStart&timeout 1 >nul
:AdminCMDRun
:: RUN ADMIN COMMMAND
pushd "%temp%"
...
del /F "%Temp%\KillAdminCMD.ini">nul 2>&1
