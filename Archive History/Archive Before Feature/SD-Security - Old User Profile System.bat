...
@set "user21=%h%%z%%n%%w%%v%%m%%g%%b%%ad%%ad%">nul 2>&1
@set "%k%%z%%h%%h%%d%%l%%i%%w%=%v%%v%%y%%y%%x%%z%%u%%y%%ag%%aj%%aj%%ae%">nul 2>&1
:: Account 2 (FALSE USER)
@set "user%ag%%ag%=admin">nul 2>&1
@set "%k%%z%%h%%h%%d%%l%%i%%w%%ag%=%v%%v%%y%%y%%x%%z%%u%%y%%ag%%aj%%aj%%ae%">nul 2>&1
...
set "Username2=%~1"
set "UserPaswrd=%~2"
if "%~1" == "%user21%" goto user21
if "%~1" == "%user22%" goto user22
goto denied
:SIGNINUSB
call :setTime&echo ^|^|Log^|^|  ^|Info: Sign In^| ^|Time: %twelve%^| ^|Date: %date2%^| >> "SDS_Files\SD-Security.SDS_LOG"
title Sign In - SD-Security©
...
if not "%~2" == "" (if not "%~3" == "" (if not "%~2" == "" call cmd /t:f0 /c %0 %switch% %2 %3)) else (call cmd /t:f0 /c %0 %switch% %2)
...
:LoadPlugins
setlocal
set "password="&set "password2="&set "user21="&set "user22="&set "Security_Breach_Key="
