...
echo New_AdminCMD_API=%New_AdminCMD_API%>> "SDS_FILES\SD-Settings.ini"
...
CHOICE /C 12345678ABCD /N /M "Ûþ "
IF ERRORLEVEL 12 %settheme%&goto :AdminOps
IF ERRORLEVEL 11 goto :SaveNewSdsSettings
IF ERRORLEVEL 10 call :ChangeTheme&goto :ChangeSettingsRefresh
IF ERRORLEVEL 9 call :ChangeBreachKey&goto :ChangeSettingsRefresh
...
:AdminCMDStart
call :AdminCMDStop
if not exist "SDS_Files\AdminCMD\" md SDS_Files\AdminCMD&if not exist "SDS_Files\AdminCMD\StartService.vbs" call :AdminCMDGenerateVBS
...
echo UAC.ShellExecute """" ^& WScript.Arguments^(0^) ^& """", "ELEV", "", "runas", 0)> "SDS_Files\AdminCMD\StartService.vbs"
goto :EOF
:AdminCMDRunCMD
...
echo echo file.writeLine HTML.ParentWindow.ClipboardData.GetData^^^("text"^^^)&echo echo file.close^)^>%%VBS%%^&goto :EOF)> "SDS_Files\AdminCMD\AdminCMD++.bat"
goto :EOF
