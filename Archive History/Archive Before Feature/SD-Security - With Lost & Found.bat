...
call :setTime&echo ^|^|Log^|^|  ^|Info: Master Override Attempted^| ^|Time: %twelve%^| ^|Date: %date2%^| >> "SDS_Files\SD-Security.SDS_LOG"
color C4
...
call :setTime&echo ^|^|Log^|^|  ^|Info: Login Failed^| ^|Username: %User%^| ^|Password: %password% ^| ^|Time: %twelve%^| ^|Date: %date2%^| >> "SDS_Files\SD-Security.SDS_LOG"
color 4C
echo.
...
echo €€€‹‹‹‹‹‹‹‹‹‹‹‹‹€€€‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹
echo.
echo                    You Are Currently Signed In As ﬂ%user%ﬂ,
...
copy SDS_Files\SD-Security.SDS_LOG "SDS_Files\tmp.txt">nul 2>&1
...
goto deletelogloop2
:CmdPlus
call :CheckStatus No "goto CmdPlus2"
call :setTime&echo ^|^|Log^|^|  ^|Info: CMD++^| ^|Time: %twelve%^| ^|Date: %date2%^| >> "SDS_Files\SD-Security.SDS_LOG"
color 07
...
call :CheckStatus No "goto :CmdPlus2"
title $Terminal
%echo% SD-Security &call :color 0a "Terminal Mode"&%echo% . [&call :color 0f "Version 1.4.1100"&%echo% ]&echo.
call :color 0b "SD-Storage"&%echo%∏ 2015, All Rights Reserved.&echo.
%echo%Type '&call :color 0e "help "&%echo% ' for the README file&echo.
:cmdCommand
echo.
:cmdCommand2
...
if /i "%CMDCmd%"=="cmd" goto :CmdPlusLoop
...
if /i "%CMDCmd%"=="sd-security" goto :home
if /i "%CMDCmd%"=="clscon" goto :clscon
if /i "%CMDCmd%"=="exit" cls&goto :cmdCommand
call :color 0b "CMD OUTPUT("&echo.&%cmdCommand%
goto :cmdCommand
:help
%echo% SD-Security &call :color 0a "Terminal Mode"&%echo% . Built-in help page:&echo.&echo.
echo    Welcome to CMD++, the portal to SD-Security's source code. With this, you
echo     can perform commands inside the framework of SD-Security, without any
echo                                  restrictions!
echo.
echo   You can perform loads of commands (this includes the vast library of API's)
echo  You can access/modify all of the data SD-Security produces and apply
echo  temp. changes to every secure variable (eg. username, password ^& status etc.)
echo.
echo      SD-Terminal overrides some of the default CMD commands (eg. cmd,help)
echo              To perform a command normally, prefix it with a space.
echo             ﬂhelpﬂ would become ﬂ helpﬂ ^& ﬂcmdﬂ would become ﬂ cmdﬂ
goto :EOF
:clscon
color 07
...
:CmdPlus2
(echo ^@echo off
echo color 07&echo set "cmd++=goto cmdCommand"&echo set "echo=<nul set /p ="&echo cls&echo :cmd
echo title $Terminal&echo %%echo%% SD-Security ^&call :color 0a "Terminal Mode"^&%%echo%% . [^&call :color 0f "Version 1.4.1000"^&%%echo%% ]^&echo.
echo echo SD-Storage∏ 2015, All Rights Reserved.&echo :cmdCommand&echo echo.&echo :cmdCommand2
echo if "%%cd:~-1%%"=="\" ^(set "Directory=%%cd%%"^) else ^(set "Directory=%%cd%%\"^)&echo set "cmdCommand="
echo %%echo%% %%directory%%^&call :color 0d "$Terminal"^&set /p "cmdCommand=>"
echo if not defined CmdCommand ^(goto :CmdCommand2^)&echo set "CMDCmd=%%cmdCommand:"=%%"
echo if /i "%%CMDCmd%%"=="cmd" goto :cmd&echo if /i "%%CMDCmd%%"=="wincmd" cmd^&goto :cmdCommand&echo if /i "%%CMDCmd%%"=="clear" cls^&goto :cmdCommand
echo if /i "%%CMDCmd%%"=="back" goto :EOF&echo if /i "%%CMDCmd%%"=="help" call :help^&goto :cmdCommand
echo if /i "%%CMDCmd%%"=="clscon" goto :clscon&echo if /i "%%CMDCmd%%"=="exit" exit 999^&goto :cmdCommand
echo call :color 0b "CMD OUTPUT("^&echo.^&%%cmdCommand%%&echo goto :cmdCommand&echo :help
echo %%echo%% SD-Security ^&call :color 0a "Terminal Mode"^&%%echo%% . Built-in help page:^&echo.^&echo.
echo echo SD-Terminal overrides some of the default commands ^(eg. cmd, help, exit etc.^)
echo echo to perform a overriden command, prefix it with a space eg. 'exit' = ' exit'
echo echo.&echo echo SD-Terminal uses multiple colors, to provide an easy to read format. To disable
echo echo the multiple colors API, simply 'set disableColor=yes'.
echo goto :EOF&echo :clscon&echo color 07&echo mode con: cols=80 lines=27&echo cls^&goto :cmd&echo :Color
echo if /i "%%disableColor%%"=="yes" %%echo%% %%~2^&goto :EOF&echo if not exist "C:\windows\system32\findstr.exe" %%echo%% %%~2^&goto :EOF
echo for /F "tokens=1,2 delims=#" %%%%a in ^('"prompt #$H#$E# & for %%%%b in (1) do rem"'^) do ^(set "DEL=%%%%a"^)
echo pushd %%temp%%&echo ^<nul set /p ".=%%DEL%%" ^> "%%~2"&echo findstr /v /a:%%1 /R "^$" "%%~2" nul&echo del "%%~2" ^> nul 2^>^&1&echo popd&echo goto :EOF
echo :AskString&echo set "StringName=%%*"&echo set "StringName=%%StringName:"=%%"
echo set /p %%StringName%%&echo for /f "tokens=1" %%%%a in ^('echo %%StringName%%'^) do set "StringName=%%%%a"^&goto AskStringNext&echo :end&echo popd
)> "%temp%\CMD++.bat"
setlocal
call :ClearPrivate
...
call :setTime&echo ^|^|Log^|^|  ^|Info: Signed In^| ^|User: %user%^| ^|Time: %twelve%^| ^|Date: %date2%^| >> "SDS_Files\SD-Security.SDS_LOG"
set "status=Signed In"
goto :EOF
...
echo Shell\Open\Command=%SDS_File%&echo Shell\Open=Launch SD-SecurityÆ
echo Action=Launch SD-SecurityÆ&echo Icon=favicon.ico
echo Label=SD-SecurityÆ&echo UseAutoplay=1
...
call :setTime&echo ^|^|Log^|^|  ^|Info: Autorun File Created^| ^|Time: %twelve%^| ^|Date: %date2%^| >> "SDS_Files\SD-Security.SDS_LOG"
goto :EOF
:makeSDSFILES
md SDS_FILES
...
echo €€€‹‹‹‹‹‹‹‹‹‹‹‹‹€€€‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹
echo.
echo                        €ﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂ€
...
:ReportCrash
@echo.
@echo €ﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂ€  €ﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂ€  €ﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂ€
...
echo €9%switch9%€  Use External Input (Delete ﬂSDS_Files\ExtInput.iniﬂ To Disable)
echo.
...
echo €€€‹‹‹‹‹‹‹‹‹‹‹‹‹€€€‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹
echo.
echo    Enter The Name Of The App You Want To Launch (Enter ﬂCancelﬂ To Cancel) :
echo      Enter ﬂUninstallﬂ To Uninstall App(s) Or Enter ﬂInfoﬂ To View App Info
echo              (To Launch An App With Full Permissions, Enter ﬂFullﬂ)
...
echo €€€‹‹‹‹‹‹‹‹‹‹‹‹‹€€€‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹
echo.
echo  Enter The Name Of The App You Want To Uninstall (Enter ﬂCancelﬂ To Cancel) :
...
echo €€€‹‹‹‹‹‹‹‹‹‹‹‹‹€€€‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹
echo.
echo     Enter The Name Of The App You Want To Info (Enter ﬂCancelﬂ To Cancel) :
...
echo €€€‹‹‹‹‹‹‹‹‹‹‹‹‹€€€‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹
echo.
echo             SD-Security Emailer Service, Enter ﬂCancelﬂ To Cancel:
...
goto :EOF
:LogSecTimer
echo ^|^|Startup Time: %TimerTime% Seconds^|^|  >> "SDS_Files\SD-Security.SDS_LOG"
goto :EOF
:ReportSecTimer
...
echo €€€‹‹‹‹‹‹‹‹‹‹‹‹‹€€€‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹
echo.
call :setTime&echo ^|^|Log^|^|  ^|Info: Debugging Stats^| ^|Time: %twelve%^| ^|Date: %date2%^| >> "SDS_Files\SD-Security.SDS_LOG"
if "%CMDSwitch%"=="" (set "CMDSwitch2=N/A") else (set "CMDSwitch2=Yes ﬂCMDSwitchﬂ")
...
echo Saved To ﬂDebugging Stats.txtﬂ!
timeout 3 >nul
goto DevOptions
:AdminCMDScreen
set "On=˛"
...
call :SetWindow
%SetTheme%
call :setTime&echo ^|^|Log^|^|  ^|Info: Sign Up Screen^| ^|Time: %twelve%^| ^|Date: %date2%^| >> "SDS_Files\SD-Security.SDS_LOG"
