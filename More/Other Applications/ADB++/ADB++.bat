...
goto :cmdCommand
:cmdCommand
if /i not "%RemoteADBInitiated%"=="yes" goto :InitiateRemoteADB
echo.
:cmdCommand2
...
:InitiateRemoteADB
echo.
echo Refreshing ADB servers...
echo.
call :color 0d "%AdminFigure%"&%echo% ^>adb kill-server&echo.
adb kill-server
call :color 0d "%AdminFigure%"&%echo% ^>adb start-server&echo.
adb start-server
call :color 0d "%AdminFigure%"&%echo% ^>adb wait-for-device&echo.
echo Please connect an Android device...
adb wait-for-device
echo Device detected!
call :color 0d "%AdminFigure%"&%echo% ^>adb usb&echo.
adb usb
call :color 0d "%AdminFigure%"&%echo% ^>adb tcpip 5555&echo.
echo Starting wireless Android server...
adb tcpip 5555
echo Finished
echo.
echo Please enter the IP address of your android device
set /p androidip="192.168."
call :color 0d "%AdminFigure%"&%echo% ^>adb connect 192.168.%androidip%&echo.
echo Connecting...
adb connect 192.168.%androidip%
echo Please disconnect Android USB
pause
call :color 0d "%AdminFigure%"&%echo% ^>adb wait-for-device&echo.
echo Detecting devices...
adb wait-for-device
echo Connected to remote ADB session, returning to CMD++ console...
timeout 3 >nul
set "RemoteADBInitiated=yes"
goto :eof
...
echo.
echo This version of CMD++ is for ADB++
echo.
