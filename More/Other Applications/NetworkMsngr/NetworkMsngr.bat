@echo off
if "%1" == "/?" call :CommandSwitchHelp&exit
set appName=%~n0
color f0
title NetworkMsngr, By SD-Storage
if exist "notifyIcon.ico" attrib +h +s "notifyIcon.ico" >nul 2>&1
if exist "newmessage.wav" attrib +h +s "newmessage.wav" >nul 2>&1
...
if not "%1" == "" goto :CommandSwitchEntered
:backFromCommandSwitch
...
:CheckForMessages
if not exist "%MessageStream%" set noStream=the message stream server&goto StreamInterrupted
if not exist "%LocalMessageStream%" set noStream=the local message stream&goto StreamInterrupted
call :Settime
...
:NewMessageSound
if /i "%Play_Sound_On_New_Message%" == "no" goto :EOF
set NoteWinTitle=New Message Sound #%random%
if exist "newmessage.wav" (call start "%NoteWinTitle%" /min %b2eextd% /play newmessage.wav)
%extd% /sleep 5
%extd% /hidewindow "%NoteWinTitle%"
goto :EOF

:StreamInterrupted
if /i "%Hide_All_Error_Popups%" == "yes" goto :CheckForMessages
%extd% /messagebox "Error - NetworkMsngr" "Failed to connect to %noStream%!" 5
...
if "%result%" == "" (goto :end) else (set MessageStream=%result%)
if not exist "%MessageStream%" goto InvalidStream
...
echo # #1 : no, *   (Default: no)>> "SDS_Files\NetworkMsngr\Settings.ini"
echo # #2 : no, yes (Default: no)>> "SDS_Files\NetworkMsngr\Settings.ini"
echo # #3 : no, yes (Default: no)>> "SDS_Files\NetworkMsngr\Settings.ini"
echo # #4 : no, yes (Default: yes)>> "SDS_Files\NetworkMsngr\Settings.ini"
echo # #5 : no, yes (Default: no)>> "SDS_Files\NetworkMsngr\Settings.ini"
echo # #6 : no, yes (Default: yes)>> "SDS_Files\NetworkMsngr\Settings.ini"
echo # #7 : 0,1,2,3,,,99999 (Default: 0)>> "SDS_Files\NetworkMsngr\Settings.ini"
...
echo Play_Sound_On_New_Message=yes>> "SDS_Files\NetworkMsngr\Settings.ini"
